...
class PraxResources(BaseModel):
    ...
    run: PraxTaskResources = Field(
        default_factory=PraxTaskResources, description="Resources for run task"
    )
    postprocess: PraxTaskResources = Field(
        default_factory=PraxTaskResources, description="Resources for postprocess task"
    )
...
class PraxConfig(BaseModel):
    ...
    pipeline_name: str = Field(
        "swan-from-rompy", description="Name of the Prax pipeline"
    )
    ...
    resources: PraxResources = Field(
        default_factory=PraxResources, description="Resource configuration"
    )
    ...
    def from_env(cls, **overrides) -> "PraxConfig":
        ...
        return cls(
            pipeline_name=overrides.get(
                "pipeline_name", os.environ.get("PRAX_PIPELINE", "swan-from-rompy")
            ),
            ...
            url=overrides.get(
                "url", os.environ.get("PRAX_URL", "https://prax.oceanum.io")
            ),
        )
...
class DataMeshConfig(BaseModel):
    ...
    tags: List[str] = Field(
        default_factory=list, description="Additional tags to apply to datasets"
    )
    labels: Dict[str, str] = Field(
        default_factory=dict, description="Labels to apply to datasets"
    )
    ...
    def from_env(cls, **overrides) -> "DataMeshConfig":
        ...
        return cls(
            enabled=overrides.get(
                "enabled", os.environ.get("DATAMESH_ENABLED", "").lower() == "true"
            ),
...
class RunConfig(BaseModel):
    ...
    executable: str = Field(
        "/usr/local/bin/swan.exe", description="Path to the model executable"
    )
    ...
    build_args: Dict[str, str] = Field(
        default_factory=dict, description="Arguments for Docker builds"
    )
...
class OceanumModelRun(BaseModel):
    ...
    prax_config: PraxConfig = Field(
        default_factory=PraxConfig.from_env, description="Prax configuration"
    )
    datamesh_config: DataMeshConfig = Field(
        default_factory=DataMeshConfig.from_env, description="DataMesh configuration"
    )
    run_config: RunConfig = Field(
        default_factory=RunConfig, description="Run configuration"
    )
    ...
    def swan_pipeline_template(self) -> Dict[str, Any]:
        ...
        if self._swan_pipeline_template is None:
            ...
            template_path = (
                pathlib.Path(__file__).parent / "templates" / "swan_pipeline.json"
            )
    ...
    def get_spec(self) -> Dict[str, Any]:
        ...
        spec = {
            **base_spec,
            ...
            "datamesh_config": (
                self.datamesh_config.dict() if self.datamesh_config else {}
            ),
        ...
        if template_path:
            with open(template_path, "r") as f:
                if template_path.endswith(".json"):
                    ...
                    raise ValueError(
                        f"Unsupported template file format: {template_path}"
                    )
        else:
    ...
    if deploy_template:
        try:
            ...
            if isinstance(model_run, OceanumModelRun):
                ...
                if not deploy_result.success:
                    logger.error(
                        f"Failed to deploy pipeline template: {deploy_result.message}"
                    )
                    return deploy_result
            else:
                logger.warning(
                    "Model run is not an OceanumModelRun, skipping template deployment"
                )
        except ImportError:
            logger.warning(
                "Could not import OceanumModelRun, skipping template deployment"
            )
