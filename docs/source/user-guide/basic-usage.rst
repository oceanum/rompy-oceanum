...
.. code-block:: bash
   ...
   # Execute model via Prax pipeline (use correct filename from init command)
   oceanum rompy run rompy_config_swan_basic.yml swan --pipeline-name my-pipeline

   # Execute with custom options
   oceanum rompy run rompy_config_swan_basic.yml swan \
...
.. note::
   ...
   **Pipeline Discovery**: Use ``oceanum rompy run --list-pipelines`` to see available pipelines in your project.

   **Common Pipeline Names for SWAN Models**:

   * ``swan-from-rompy`` - Basic SWAN execution pipeline
   * ``swan-rompy-basic`` - Standard SWAN with validation
   * ``swan-rompy-operational`` - Optimized for operational forecasting

   **Pipeline Not Found Error**: If you get a "pipeline not found" error, either:

   1. List available pipelines: ``oceanum rompy run --list-pipelines``
   2. Use an existing pipeline name from step 1
   3. Deploy a new pipeline using ``--deploy --template <template_file>``
...
.. code-block:: bash
   ...
   # 3. Execute model via Prax pipeline (note: use the generated filename)
   oceanum rompy run rompy_config_swan_basic.yml swan --pipeline-name my-pipeline
...
.. code-block:: bash
   ...
   # Submit pipeline execution (use filename from init command)
   oceanum rompy run rompy_config_swan_basic.yml swan --pipeline-name my-pipeline
...
The oceanum CLI provides clear error messages and troubleshooting guidance:

.. code-block:: bash

   # File not found errors (common issue)
   $ oceanum rompy run config.yml swan --pipeline-name test
   ❌ Error parsing configuration: [Errno 2] No such file or directory: 'config.yml'
   💡 Make sure your configuration file exists and contains valid YAML or JSON
   📁 Looking for: config.yml

**Solution**: Use the correct filename generated by ``oceanum rompy init``, for example:

.. code-block:: bash

   # After running: oceanum rompy init swan --template basic
   oceanum rompy run rompy_config_swan_basic.yml swan --pipeline-name my-pipeline

.. code-block:: bash

   # Configuration parsing errors
   $ oceanum rompy run malformed.yml swan --pipeline-name test
   ❌ Error parsing configuration: Configuration must be a valid YAML dictionary
   💡 Make sure your configuration file exists and contains valid YAML or JSON
